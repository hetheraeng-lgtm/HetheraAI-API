...
from app.schema.common import ApiResponse, ChatIdPath, MobileDataServiceIdPath
...
router = APIRouter(prefix="/mobile-data", tags=["Mobile Data"])
...
def _get_mobile_data_service(
    session: AsyncSession = Depends(get_db),
    vtpass: VtpassService = Depends(_get_vtpass),
) -> MobileDataService:
    return MobileDataService(session, vtpass)
...
@router.get(
    ...
    description="Returns all supported mobile data provider IDs.",
)
async def list_providers(
    service: MobileDataService = Depends(_get_mobile_data_service),
...
@router.get(
    ...
    description="Fetch available data bundles for a provider from VTpass.",
)
async def get_variation_codes(
    service_id: MobileDataServiceIdPath,
    service: MobileDataService = Depends(_get_mobile_data_service),
) -> ApiResponse[dict]:
...
@router.post(
    ...
    description="Purchase a mobile data bundle for the given phone number using a saved card.",
)
async def buy_mobile_data(
    chat_id: ChatIdPath,
    payload: MobileDataPurchaseRequest,
    service: MobileDataService = Depends(_get_mobile_data_service),
    ...
    return ApiResponse(
        message="Mobile data purchase processed",
